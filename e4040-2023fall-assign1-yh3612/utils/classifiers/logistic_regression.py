...
def logistic_regression_loss_vectorized(w, X, y, reg):
    ...
    loss_positive = -np.log(probabilities[y == 1])
    loss_negative = -np.log(1 - probabilities[y == 0])
    loss = np.sum(loss_positive) + np.sum(loss_negative)
    loss /= N
    loss += 0.5 * reg * np.sum(w * w)
