...
def conv2d_forward(x, w, b, pad, stride):
    ...
    for i in range(0, new_height * stride, stride):
        for j in range(0, new_width * stride, stride):
            x_slice = x_padded[:, i:i+filter_height, j:j+filter_width, :]
            
            for k in range(num_of_filters):
                out[:, i // stride, j // stride, k] = np.sum(x_slice * w[:, :, :, k], axis=(1,2,3)) + b[k]
...
def max_pool_backward(dout, x, pool_size, stride):
    ...
    # raise NotImplementedError
    batch, height, width, channels = x.shape
    _, new_height, new_width, _ = dout.shape
    dx = np.zeros_like(x)
        ...
        for j in range(0, new_width):
            for b in range(batch):
                for c in range(channels):
                    x_slice = x[b, i*stride:i*stride+pool_size, j*stride:j*stride+pool_size, c]
                    mask = (x_slice == np.max(x_slice))
                    dx[b, i*stride:i*stride+pool_size, j*stride:j*stride+pool_size, c] += mask * dout[b, i, j, c]
