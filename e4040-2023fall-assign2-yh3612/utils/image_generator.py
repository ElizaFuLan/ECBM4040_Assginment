...
class ImageGenerator(object):
    def __init__(self, x, y):
        ...
        self.x = x.copy()
        self.y = y.copy()
        self.N, self.height, self.width, self.channels = self.x.shape
    ...
    def next_batch_gen(self, batch_size, shuffle=True):
        ...
        #######################################################################
        # raise NotImplementedError
        total_batches = self.N // batch_size
        batch_count = 0
    ...
    def add_noise(self, portion, amplitude):
        ...
        added = self.x.copy()
        for i in index:
            added[i, :, :, :] += np.random.randint(0, 5, [self.height, self.width, self.channels], dtype='uint8') * amplitude
