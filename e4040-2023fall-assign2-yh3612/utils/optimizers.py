...
class SGDNestMomentumOptim(SGDMomentumOptim):
    def step(self, learning_rate):
        ...
        for k in params:
            velocities[k] = momentum * velocities[k] + grads[k]
            nesterov_velocity = momentum * velocities[k] + grads[k]
            params[k] -= learning_rate * nesterov_velocity
        ###################################################
        #               END OF YOUR CODE                  #
        ###################################################
...
class AdamOptim(Optimizer):
    ...
    def step(self, learning_rate):
        ...
        for k in params:
            ...
            cor_mean = mean[k] / (1 - beta1 ** t)
            cor_var = var[k] / (1 - beta2 ** t)
            params[k] -= learning_rate  / (np.sqrt(cor_var) + eps) * cor_mean
...
class BacktraceSGDOptim(SGDOptim):
    ...
    def step(self, learning_rate):
        ...
        for i in range(1, max_searches):
            ...
            armijo_upper_bound = loss_curr - alpha * G
            ...
            if loss_next <= armijo_upper_bound:
                break
            ###################################################
            #               END OF YOUR CODE                  #
            ###################################################
