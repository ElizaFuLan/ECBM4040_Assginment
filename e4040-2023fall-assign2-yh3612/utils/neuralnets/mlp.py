...
class MLP:
    ...
    def loss(self, preds, y):
        ...
        for i in range(num_layers - 1):
            ...
            #raise NotImplementedError
            if dropout_config["enabled"]:
                dx = dropout_backward(dx, cache["dropout_{}".format(j)]) # ADD HERE!
            ###############################################
            # END OF DROPOUT                              #
            ###############################################
            ...
            ###############################################
            # TODO: Add batch normalization here          #
            #       Remeber to store the gradients        #
            ###############################################
            #raise NotImplementedError
            if use_bn:
        ...
        # gradients are accumulative in nature
        # need to be manually cleared by the optimizers
        for k in self.grads:
